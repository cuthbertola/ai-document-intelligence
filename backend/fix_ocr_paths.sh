...
# Fix OCR Path Configuration Script
echo "🔧 Fixing OCR path configuration..."

# 1. Config is already updated (confirmed)
echo "✅ Config already updated with correct path"
grep PROCESSED_FOLDER ~/ai-document-platform/backend/app/core/config.py

# 2. Create Python script to generate missing .txt files
echo ""
echo "📄 Creating script to generate missing text files..."

cat > ~/ai-document-platform/backend/generate_missing_txt.py << 'PYTHON_EOF'
import os
